...
function parseAircraftState(state) {
    return {
        ...
        on_ground: state[8] === true || state[8] === 1,
...
export async function GET() {
    try {
        ...
        if (!response.ok) {
            return NextResponse.json({ error: "Failed to fetch aviation data" }, { status: 502 });
        }
        ...
        const features = states
            ...
            .map((s) => {
                ...
                return {
                    ...
                    geometry: {
                        type: "Point",
                        coordinates: [ac.longitude, ac.latitude, ac.altitude || 0],
                    },
                };
            });
        ...
        return NextResponse.json({
            ...
            features,
            metadata: { count: features.length, time: data?.time },
        });
    } catch (error) {
