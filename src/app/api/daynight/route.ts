...
export async function GET() {
    try {
        ...
        return NextResponse.json({
            ...
            features,
            metadata: { count: features.length, source: "computed", note: "Mathematical day/night terminator" },
        });
    } catch (error) {
        console.error("[DayNightRoute] Error:", error);
        return NextResponse.json({ error: "Failed to compute day/night data" }, { status: 502 });
    }
}
