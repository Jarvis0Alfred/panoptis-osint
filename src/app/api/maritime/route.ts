...
export async function GET() {
    try {
        ...
        return NextResponse.json({
            ...
            features,
            metadata: { count: features.length, source: "sample-data", note: "Replace with live AIS API for production" },
        });
    } catch (error) {
        console.error("[MaritimeRoute] Error:", error);
        return NextResponse.json({ error: "Failed to fetch maritime data" }, { status: 502 });
    }
}
