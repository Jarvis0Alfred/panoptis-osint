...
export async function GET() {
    try {
        const features = WILDFIRES.map((f) => ({
            type: "Feature",
            properties: {
                id: f.id,
                name: f.name,
        ...
        return NextResponse.json({
            ...
            features,
            metadata: { count: features.length, source: "nasa-firms-sample", note: "Replace with live NASA FIRMS API for production" },
        });
    } catch (error) {
        console.error("[WildfireRoute] Error:", error);
        return NextResponse.json({ error: "Failed to fetch wildfire data" }, { status: 502 });
    }
}
